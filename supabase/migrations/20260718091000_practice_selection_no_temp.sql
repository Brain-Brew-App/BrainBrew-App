-- BrainBrew — Practice selection hardening (Phase 7B).
--
-- Replace the temporary-table selection with a single INSERT … SELECT. A
-- `create temporary table … on commit drop` inside a SECURITY DEFINER function is
-- unsafe under PgBouncer/connection pooling (it can linger on a pooled connection
-- and collide on the next call). This version has no session state: it selects,
-- inserts the five slots, and checks the row count in one statement — an
-- insufficient pool raises (rolling the whole start back). Behaviour is identical.
...
begin
  ...
  -- Select five eligible reserve puzzles (one per category) and insert them in one
  -- statement. Hard constraints: approved, validator-passed, active engine,
  -- supported version, never today's ranked. Reserve is a strong soft preference.
  with excluded as (
  ...
  if cnt < 5 then
    raise exception 'practice_pool_exhausted' using errcode = 'P0001';  -- rolls back the pack too
  end if;
