-- RC1.1 — corrective replace of open_slot_for_attempt.
-- The first version declared a local named `slot_id`, which is ambiguous against
-- attempt_items.slot_id inside the function body (Postgres rejects it at runtime).
-- Locals are now v_-prefixed and the item lookup is table-qualified. Same behaviour,
-- same checks, same payload — see 20260802090000 for the full rationale.
-- WHY
-- ---
